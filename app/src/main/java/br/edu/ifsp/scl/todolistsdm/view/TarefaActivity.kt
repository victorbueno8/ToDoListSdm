...
import br.edu.ifsp.scl.todolistsdm.model.database.ToDoListDatabase_Impl
...
class TarefaActivity : AppCompatActivity() {
    ...
    private inner class SalvarAtualizarTarefaAT: AsyncTask<Tarefa, Unit, Tarefa>() {
        override fun onPostExecute(result: Tarefa?) {
            super.onPostExecute(result)
            if (result != null) {
                val intentRetorno = Intent()

                intentRetorno.putExtra(MainActivity.Constantes.TAREFA_EXTRA, result)
